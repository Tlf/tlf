...
while (1)  {
	...
	if (strstr(spot_ptr[i], "DX de") != NULL) {
	   ...
	   if (cluster == MAP) {
		...
		{
							case 1800 ... 1850:
								if (bandinx==0) in_map = 1;
								break;

							case 3500 ... 4000:
								if (bandinx==1) in_map = 1;
								break;

							case 7000 ... 7200:
								if (bandinx==2) in_map = 1;
								break;

							case 10100 ... 10150:
								if (bandinx==3) in_map = 1;
								break;

							case 14000 ... 14350:
								if (bandinx==4) in_map = 1;
								break;

							case 18068 ... 18168:
								if (bandinx==5) in_map = 1;
								break;

							case 21000 ... 21450:
								if (bandinx==6) in_map = 1;
								break;

							case 24900 ... 24950:
								if (bandinx==7) in_map = 1;
								break;

							case 28000 ... 29600:
								if (bandinx==8) in_map = 1;
								break;
			...
			i++;
				...
				i++;
				 ...
				 i++;
	}  else   if ((cluster == CLUSTER) && (announcefilter == 0) && (strlen(spot_ptr[i]) > 20)){
...
while (temps != NULL) {
	...
	if (thisline == NULL) break;
		...
		if (strncmp(thisline, "DX de ",6) == 0) {
			...
			if (timediff + 30 < 0) timediff += 1440;

			if ((timediff+30) <= (MAXMINUTES + 30 )) recent = 1;
			else {
				recent = 0;
				thisline[0] = 'd';
			}
...
if (cluster == MAP) linepos =bandmap_pos;
	...
	if (bandmap[j]){
		...
		strcat (spotline, spottime);
			...
			if (x != 0 && xplanet > 0 && nofile == 0) {
				...
				marker_out[12] = '\0';
				strcat(marker_out, "   \"");
				if (xplanet == 1 || xplanet == 3) strcat(marker_out, callcopy);
				if (spot_age[j] > 15 && cluster != SPOTS) strcat(marker_out, "\"   color=Green\n");
				else {

					iswarc = 0;
					if (spot_freq[j] >= 10100.0 && spot_freq[j] <= 10150.0) iswarc = 1;
					if (spot_freq[j] >= 18068.0 && spot_freq[j] <= 18168.0) iswarc = 1;
					if (spot_freq[j] >= 24890.0 && spot_freq[j] <= 24990.0) iswarc = 1;
					...
					if (iswarc == 0) {
						if (cluster == MAP){
							...
							if (spot_freq[j] >= 3500.0 && spot_freq[j] <= 4000.0)
								strcat(marker_out, "\"   color=Magenta\n");
							if (spot_freq[j] >= 7000.0 && spot_freq[j] <= 7300.0)
								strcat(marker_out, "\"   color=Yellow\n");
							if (spot_freq[j] >= 14000.0 && spot_freq[j] <= 14350.0)
								strcat(marker_out, "\"   color=Blue\n");
							if (spot_freq[j] >= 21000.0 && spot_freq[j] <= 21450.0)
								strcat(marker_out, "\"   color=White\n");
							if (spot_freq[j] >= 28000.0 && spot_freq[j] <= 29000.0)
...
//						iswarc = 0;
	...
	if ( xplanet == 1 && nofile == 0) {
		...
		if  ( (fp = fopen(markerfile,"a"))  == NULL){
			...
			if (strlen(xplanetmsg) > 20) fputs( xplanetmsg, fp);

			fclose(fp);
		}
	}

	if (cluster == MAP && allspots == 1)	nicebox(14,  3, 8,  27,  "Bandmap");
	if (cluster == MAP && allspots == 0)	nicebox(14,  3, 8,  27,  "Needed");

	if (cluster == SPOTS)   nicebox(14,  3, 8,  27,  "Spots");
